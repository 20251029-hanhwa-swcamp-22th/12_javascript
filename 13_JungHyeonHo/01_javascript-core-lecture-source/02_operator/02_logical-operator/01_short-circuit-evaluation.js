...
// 1) 'apple'은 Truthy한 값이기 때문에 true로 평가됐다 
// 2) 평가 도중 true가 발생하면(확정되면) 나머지 연산을 수행하지 않는다
...
// 1) 'apple'이 true로 평가 되었다.
// 2) &&연산은 좌항, 우항을 모두 평가해야된다.
// 3) 우항이 평가의 결과를 결정함
//      -> 좌항 true면 우항이 반환됨
console.log('apple' && 'banana'); // banana
console.log('apple' && false); // false
// 앞쪽이 false면 앞에 값이 결과를 결정
console.log(false && 'banana'); // false

// var obj = null;
// var temp = obj.value;
// console.log(temp); // TypeError: Cannot read properties of null (reading 'value')

// 객체를 가리키기를 기대하는 변수가 null 또는 undefined인지 확인하고
// null 또는 undefined (Falsy)로 판단되면
// 객체의 속성(프로퍼티) 참조를 막아서 에러를 예방
var obj = null;
var temp = obj && obj.value; // &&로 인해서 null은 falsy기 때문에 temp에 null이 담긴다
