...
const theater = {
    ...
    showMovieList(){
        ...
        this.titles.forEach(function(title){
            // 여기에서 this는
            // theater가 아니라 function
            // 일반 함수 내에서 this == 함수 객체 자체를 나타냄
            console.log(this.store,title); //this.store == defined
        });

        // 화살표 함수를 이용한 forEach
        // 화살표 함수는 this를 ()=>{} 표현식으로 만든 함수 객체와 연결하지 않는다.
        // -> 화살표 함수 밖에 있는 theater 객체가 this와 매핑된다.
        this.titles.forEach((title)=>{console.log(this.store,title);}) //this.store == 강남점
    }
};
...
const testFn = ()=>{
    console.log(this); // this == global 객체(브라우저에서는 window 객체)
}
...
// arguments키워드는 자바스크립트에서 매개변수가 하나이상 선언되어있는데
// 해당 값 이상의 값을 넘겨받으면
// 모든 값은 arguments에서 관리한다.
(function() {
