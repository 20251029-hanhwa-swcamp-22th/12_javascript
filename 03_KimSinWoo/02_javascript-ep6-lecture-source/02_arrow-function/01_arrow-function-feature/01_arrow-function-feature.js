// 화살표 함수는 기존 함수보다 표현식이 간략화 된 것 뿐만 아니라
// 내부 동작 또한 간략화 되었다.


// 1. 화살표 함수는 this를 가리지 않는다
const theater= {
    store: '강남점',
    titles: ['만약에우리','아바타3','주토피아'],

    showMovieList() {
        // 일반 함수를 이용한 forEach
        this.titles.forEach(function (title){
            // 일반 함수 내에서 this == 함수 객체 자체를 나타낸다
            console.log(this.store, title); // -> this.store: undefined
        });

        // 화살표 함수를 이용한 forEach
        this.titles.forEach((title)=>{
            // 화살표 함수는 this를 () => {} 표현식으로 만든 함수 객체와 연걸하지 않는다.
...
// const arrowFunc = () => {}; // 화살표 함수는 생성자 함수로 인식X
