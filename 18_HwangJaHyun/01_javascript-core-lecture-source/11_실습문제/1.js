/* 1-1. 전달 받은 문자열(str) 안에 있는 값을
',' 기준으로 배열로 분리하고 전달받은 단어(word)가 있는 
...
function findWord(str, word) {
    ...
    for(let i = 0; i < strArr.length; i++){
        ...
        }else if(strArr[i] === word){
            return `${word}는 배열 index ${i}번째에 있습니다.`;
        }
    }
...
/* 1-3. 제시 된 숫자값 배열과 문자값 배열을 
오름차순 정렬, 내림차순 정렬하여 출력한다. */
...
/* 1-4. Student 생성자 함수를 통해 생성 된 
3명의 학생 객체를 studentList 배열에 담았다. 
해당 배열을 전달하여 score 내림차순 기준으로 
정렬해주는 sortFromScore(arr) 함수, 
해당 배열을 전달하여 lastName과 firstName을 합성하여 
name 속성으로 만들어 반환하는 makeFullName 함수를 작성한다. */
...
function makeFullName(arr){
    ...
    for(let i = 0; i < arr.length; i++){
        arr[i].name = arr[i].lastName + arr[i].firstName;
    }

    return arr.map(item => ({name: item.name, score: item.score}));
}