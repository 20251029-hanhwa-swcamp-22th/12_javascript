...
//   반환되는 값들로만 이루어진 새 배열을 반환하는 고차 함수
...
const lens = original2.map(item => item.length);
...
let result2 = testArr.some(item => item > 3);
...
// 배열 내 요소 중 10보다 작지않은 요소가 하나라도 있는가?
let result3 = testArr.every(item => item > 0);
console.log("result3 : ", result3);

let result4 = testArr.every(item => item > 2);
console.log("result4 : ", result4);

// Array.prototype.find  :
// 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여
// 그 결과가 참인 첫번째요소를 반환참인 요소가 존재하지 않는다면 undefined를 반환
...
// 점수가 80점 이상인 학생 이름만 모두 찾기(1개 == find, n개 == filter)
result = students
